...
export default function ManageDishes() {
  ...
  const [form, setForm] = useState({ name: '', category: 'Biryani', price: '', available: true, image: '' });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editing) {
        await axios.put(`${API}/dishes/${editing}`, payload, getHeaders());
        toast.success('Dish updated!');
      } else {
        await axios.post(`${API}/dishes`, payload, getHeaders());
        toast.success('Dish added!');
      }
      setShowForm(false);
      setEditing(null);
      setForm({ name: '', category: 'Biryani', price: '', available: true, image: '' });
      fetchData();
    } catch (err) {
      ...
      <div className="container">
        ...
        <div className="d-flex justify-content-between align-items-center mb-4">
          ...
          <button className="btn btn-primary-custom" style={{ width: 'auto', padding: '10px 25px' }}
            onClick={() => { setShowForm(!showForm); setEditing(null); setForm({ name: '', category: 'Biryani', price: '', available: true, image: '' }); }}>
            ...
            {dishes.map((dish) => (
              <div key={dish._id} className="col-md-3 col-sm-6">
                <div className="card dish-card">
                  {dish.image ? (
                    <img src={getImageUrl(dish.image, dish.updatedAt || dish._id)} alt={dish.name} className="card-img-top" />
                  ) : (
