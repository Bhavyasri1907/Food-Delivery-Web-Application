...
export default function ManageRestaurants() {
  ...
  const [form, setForm] = useState({ name: '', city: '', description: '', image: '', allowCancellation: true });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editing) {
        await axios.put(`${API}/restaurants/${editing}`, payload, getHeaders());
        toast.success('Restaurant updated!');
      } else {
        await axios.post(`${API}/restaurants`, payload, getHeaders());
        toast.success('Restaurant added!');
      }
      setShowForm(false);
      setEditing(null);
      setForm({ name: '', city: '', description: '', image: '', allowCancellation: true });
      fetchRestaurants();
    } catch (err) {
    ...
    <div className="page-content">
      <div className="container">
        <div className="d-flex justify-content-between align-items-center mb-4">
          <h2 className="fw-bold">All Restaurants</h2>
          <button className="btn btn-primary-custom" style={{ width: 'auto', padding: '10px 25px' }}
            onClick={() => { setShowForm(!showForm); setEditing(null); setForm({ name: '', city: '', description: '', image: '', allowCancellation: true }); }}>
            ...
            {restaurants.map((r) => (
              <div key={r._id} className="col-md-4 col-sm-6">
                <div className="card restaurant-card" style={{ cursor: 'default' }}>
                  {r.image ? (
                    <img src={getImageUrl(r.image, r.updatedAt || r._id)} alt={r.name} className="card-img-top" />
                  ) : (
