...
export default function CustomerHome() {
  ...
  useEffect(() => {
    // Load all restaurants by default; apply city filter only when selected.
    fetchRestaurants();
    // eslint-disable-next-line
  }, [selectedCity]);
  ...
  const fetchCities = async () => {
    try {
      ...
      console.error(err);
      toast.error('Unable to load cities. Please check backend connection.');
    }
  };
  ...
  const fetchRestaurants = async () => {
    ...
    try {
      ...
      console.error(err);
      toast.error('Unable to load restaurants. Please check backend/API URL.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleCategoryClick = async (categoryName) => {
    ...
    try {
      ...
      console.error(err);
      toast.error('Unable to load category dishes.');
    } finally {
                  ...
                  <div key={dish._id} className="col-lg-3 col-md-4 col-sm-6">
                    <div className="card category-dish-card">
                      <div className="category-dish-img-wrapper">
                        {dish.image ? (
                          <img
                            src={getImageUrl(dish.image, dish.updatedAt || dish._id)}
                            alt={dish.name}
                            className="card-img-top"
                            onError={(e) => handleImageError(e, `dish-${dish.name}`)}
                          />
                        ) : (
              ...
              <div key={restaurant._id} className="col-lg-4 col-md-6">
                ...
                >
                  <div className="card-img-wrapper">
                    {restaurant.image ? (
                      <img
                        src={getImageUrl(restaurant.image, restaurant.updatedAt || restaurant._id)}
                        alt={restaurant.name}
                        className="card-img-top"
                        onError={(e) => handleImageError(e, `restaurant-${restaurant.name}`)}
                      />
                    ) : (
