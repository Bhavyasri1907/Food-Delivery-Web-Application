...
const CATEGORIES = ['All', 'Biryani', 'Meals', 'Snacks', 'Fast Food', 'Beverages', 'Desserts'];
...
export default function RestaurantMenu() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      // Exclude dishes categorized as 'Other' from customer view
      setDishes(dishRes.data.filter(d => d.category !== 'Other'));
    } catch (err) {
      console.error(err);
    } finally {
      setLoading(false);
                ...
                <div className="card dish-card">
                  {dish.image ? (
                    <img
                      ...
                      onError={(e) => handleImageError(e, `menu-dish-${dish.name}`)}
                    />
                  ) : (
